document.addEventListener('DOMContentLoaded', () => {
  ...
  const populateForm = (data) => {
    ...
    if (comorb.temComorbidade) {
        ...
        if (comorb.lista) {
            checkboxesComorbidades.forEach(cb => { if(comorb.lista.includes(cb.value)) cb.checked = true; });
        }
    } else {
    ...
    list.forEach(evo => {
        ...
        const html = `
            <div class="evolucao-item" id="evo-${evo._id}">
                <div class="evo-header" id="header-${evo._id}" onclick="toggleEvolucao('${evo._id}')">
                    <div class="evo-left"><span class="evo-date">${dataStr}</span><strong class="evo-title">${evo.titulo}</strong></div>
                    <div class="evo-right">
                ...
                <div class="evo-body" id="body-${evo._id}">
                    <p>${evo.texto.replace(/\n/g, '<br>')}</p>
                    <small style="color:#aaa; display:block; margin-top:5px;">Ass: ${evo.autor || 'Dra. Aisha'}</small>
                </div>
            </div>`;
  ...
  window.toggleEvolucao = (id) => {
      ...
      if (body.classList.contains('aberto')) { body.classList.remove('aberto'); header.classList.remove('aberto'); } 
      else { body.classList.add('aberto'); header.classList.add('aberto'); }
  };

  window.startEditEvolucao = (id) => {
      const evo = currentEvolucoes.find(e => e._id === id); if (!evo) return;
      tituloEvolucaoInput.value = evo.titulo; textoEvolucaoInput.value = evo.texto; editingEvolucaoId = id;
      btnAddEvolucao.innerText = 'Salvar Alteração'; btnAddEvolucao.style.backgroundColor = '#FFB74D';
  ...
  window.deleteEvolucao = async (id) => {
      ...
      try {
          const res = await fetch(`${API_ADMIN_BASE}${pacienteId}/evolucao/${id}`, { method: 'DELETE', headers: {'Authorization': `Bearer ${token}`} });
          if (res.ok) { 
              const data = await res.json(); 
              if(data.prontuario) currentEvolucoes = data.prontuario.evolucoes; 
              renderEvolucoes(); 
          }
  ...
  const handleSaveEvolucao = async () => {
      const titulo = tituloEvolucaoInput.value.trim(); const texto = textoEvolucaoInput.value.trim();
      ...
      try {
          ...
          const res = await fetch(url, { method, headers: {'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`}, body: JSON.stringify({ titulo, texto }) });
          const data = await res.json();
          if (res.ok) { 
              ...
              btnAddEvolucao.innerText = '+ Registrar Evolução'; btnAddEvolucao.style.backgroundColor = '#2ADCA1';
          }
      } catch (e) { alert('Erro.'); }
